  ...
  const onRoleBindingChange = useCallback((rb: RoleBinding[] | ClusterRoleBinding) => {
    if (rb) {
      const namespaces: string[] = Array.isArray(rb)
        ? [...new Set((rb as RoleBinding[]).filter((rb) => rb.namespace).map((rb) => rb.namespace))]
        : [...new Set((rb as ClusterRoleBinding).subjects?.filter((s) => s.namespace).map((s) => s.namespace))].filter(
            (e) => e !== undefined
          )
